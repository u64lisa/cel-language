...
public class LLVMTypeLookup {

    final Map<String, Type> types;

    public LLVMTypeLookup() {
    ...
    public Type resolve(Node statement) {
        switch (statement.getNodeType()) {
            ...
            case CLASS_ACCESS -> {
                ...
                if (attrType == null) {
                    error("Attribute", String.format("'%s' object has no attribute '%s'", clazz, attr));
                }
                return attrType;
            }
    ...
    private Type resolve(BinOpNode statement) {
        ...
        if (result == null) {
            error("Type", String.format("Cannot apply '%s' to '%s' and '%s'", statement.operation,
                    left, right));
        }
        return result;
    }
    ...
    private Type resolve(UnaryOpNode statement) {
        ...
        if (result == null) {
            error("Type", String.format("Cannot apply '%s' to '%s'", statement.operation, right));
        }
        return result;
    }
    ...
    private Type resolve(CallNode node) {
        ...
        if (result == null) {
            error("Call", "Cannot call function with the given arguments");
        }
        return result;
    }
    ...
    private Type resolve(ReturnNode node) {
        ...
        //    error("Return", "Return type must be the same as the function's return type");
        //}
        return Types.VOID;
    }
    ...
    private Type resolve(ClassDefNode node) {
        ...
        if (node.parentToken != null) {
            ...
            if (unknownType == null) {
                error("Class", "Parent class must be defined before it is used");
            }
            if (!(unknownType instanceof ClassType)) {
                error("Class", "Parent must be a class");
            }
    ...
    private Type resolve(DerefNode node) {
        ...
        if (!(dereferencing instanceof ReferenceType)) {
            error("Deref", "Cannot dereference non-reference type");
        }
